...
/*   Updated: 2022/08/25 18:20:06 by tdehne           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void	coloring(void *context)
{
	...
	while (tmp->px != -1)
	{
		...
		else if (all_s->fractal->frac_type == MANDEL
			|| all_s->fractal->frac_type == JULIA)
			color_mandel(tmp, all_s);
		else if (all_s->fractal->frac_type == BSHIP)
			color_bship(tmp, all_s);
		(tmp)++;
	}
...
void my_keyhook(mlx_key_data_t keydata, void* context)
{
	...
	if (mlx_is_key_down(all_s->vars->mlx, MLX_KEY_RIGHT))
		...
		all_s->g_vars->offset_y += 0.1 * all_s->g_vars->steps_y * all_s->vars->win_height;
	else if (mlx_is_key_down(all_s->vars->mlx, MLX_KEY_A))
		all_s->vars->iter_depth += 50;
	else if (mlx_is_key_down(all_s->vars->mlx, MLX_KEY_S))
		all_s->vars->iter_depth -= 50;
	else if (mlx_is_key_down(all_s->vars->mlx, MLX_KEY_1))
